...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isN8nMockEnabled()) {
      ...
      const mockData = buildMockRfpOpenResponse(body, correlationId);
    ...
    const url = `${N8N_WEBHOOK_BASE}/rfp/open`;
    ...
    console.error('Unexpected error in POST /api/workflows/rfp-open:', error);
